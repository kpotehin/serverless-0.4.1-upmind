                            ...
                            if (SUtils.fileExistsSync(path.join(fPath2, 's-function.json'))) {
                                ...
                                return BbPromise.resolve(fs.readdirSync(path.join(_this._config.fullPath, sf, sf2)))
                                    .each(function(sf3) {
                                        ...
                                        // If s-function.json doesn't exist, look in 3 subfolders
                                        if (SUtils.fileExistsSync(path.join(fPath3, 's-function.json'))) {
                                            ...
                                            // Loop through 3rd level subfolders looking for more functions
                                            return BbPromise.resolve(fs.readdirSync(path.join(_this._config.fullPath, sf, sf2, sf3)))
                                                .each(function(sf4) {

                                                    let fPath4 = path.join(fPath3, sf4);
                                                    // Skip reserved names and files
                                                    if (reserved.indexOf(sf4.trim()) !== -1 || !fs.lstatSync(fPath4).isDirectory()) return;

                                                    // If s-function.json doesn't exist, look in 4 subfolders
                                                    if (SUtils.fileExistsSync(path.join(fPath4, 's-function.json'))) {
                                                        let fnJson = SUtils.readAndParseJsonSync(path.join(fPath4, 's-function.json'));
                                                        return loadFn(_this._config.sPath + '/' + sf + '/' + sf2 + '/' + sf3 + '/' + fnJson.name);
                                                    }
                                                });
                                        }
                                    });
