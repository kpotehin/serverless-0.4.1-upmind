...
function validatePath(currentPath, functionPath) {
  var index = functionPath.indexOf(currentPath);
  var lastSymbol = functionPath[currentPath.length];
  return index === 0 && (
    lastSymbol === '@' ||
    lastSymbol === '/' ||
...
class ServerlessState {
  ...
  getFunctions(options) {
    ...
    for (let i = 0; i < allFunctions.length; i++) {
      ...
      for (let j = 0; j < options.paths.length; j++) {
        if (validatePath(options.paths[j], func._config.sPath)) {
  ...
  getEndpoints(options) {
    ...
    for (let i = 0; i < allEndpoints.length; i++) {
      ...
      for (let j = 0; j < options.paths.length; j++) {
        if (validatePath(options.paths[j], endpoint._config.sPath)) {
