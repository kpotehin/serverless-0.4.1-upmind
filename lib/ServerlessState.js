...
class ServerlessState {
  ...
  getFunctions(options) {
    ...
    for (let i = 0; i < allFunctions.length; i++) {
      ...
      for (let j = 0; j < options.paths.length; j++) {
        if (
          func._config.sPath.indexOf(options.paths[j]) !== -1 &&
          func._config.sPath.split('@')[0] === options.paths[j].split('@')[0]
        ) {
  ...
  getEndpoints(options) {
    ...
    for (let i = 0; i < allEndpoints.length; i++) {
      ...
      for (let j = 0; j < options.paths.length; j++) {
        if (
          endpoint._config.sPath.indexOf(options.paths[j]) !== -1 &&
          options.paths[j].split('@')[0] === endpoint._config.sPath.split('@')[0]
        ) {
