...
class ServerlessFunction {
  ...
  getDeployedName(options) {
    ...
    // if (this._config.sPath.split('/').length == 3) {
      ...
      if (SUtils.fileExistsSync(path.join(
              this._S.config.projectPath,
              this._config.sPath.split('/').splice(0, 3).join(path.sep),
              's-module.json'))) {
        name = name + '-' + this._config.sPath.split('/')[2];
      }

      if (SUtils.fileExistsSync(path.join(
              this._S.config.projectPath,
              this._config.sPath.split('/').splice(0, 4).join(path.sep),
              's-module.json'))) {
        name = name + '-' + this._config.sPath.split('/')[3];
      }
    // }
